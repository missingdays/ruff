...
use lsp_types::request::{Request, SelectionRangeRequest};
use lsp_types::{Position, Range, SelectionRangeParams, TextDocumentIdentifier, Url};
...
use ruff_python_ast::AnyNodeRef::ExprAttribute;
...
use crate::document::{PositionExt, RangeExt};
...
impl BackgroundDocumentRequestHandler for ProvideTypeRequestHandler {
    ...
    fn run_with_snapshot(
        ...
        let range_offset = params.range.to_text_range(
            &source,
            &line_index,
            snapshot.encoding(),
        );
        ...
        Ok(
            Some(
                ProvideTypeResponse { ty: name.to_string() }
            )
        )
    }
}
